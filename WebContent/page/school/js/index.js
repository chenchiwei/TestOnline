...
testonline.school= {
	...
	init : function(){
		...
		/*testonline.school.setSearchBtn();*/
		/*$.ajax({
			type:'post',
...
//					"'waterHeight':56,'takeUpAcreage':80000,'typhoonLevel':12,'tideType':'半日潮','seareaType':'内海','feedClass':'大鱼'}",page:1,rows:10}
	...
	setDataGrig : function(){
		$('#data').datagrid({  
            ...
            columns:[[
                      {field:'schoolId',title:'编号',width:100,align:'center'},
                      {field:'schoolName',title:'名称',width:100,align:'center'},
                      {field:'schoolDesc',title:'描述',width:100,align:'center'},
                  ]],
            pagination:true,
            pageSize:15,
            ...
            onLoadSuccess : function(data){
            	...
            	if(data.total == 0){
            		if(testonline.school.searchStat == '查询'){
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">您查询的学校信息不存在！</p>');
            			testonline.school.searchStat='';
            		}else{
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">暂无学校信息！</p>');
            		}
            	}else{
				...
				$.ajax({
					...
					success : function(result){
						if(result.success){
							...
							$('#fm').form('clear');
							$("#submitBtn").attr("onclick","testonline.school.editSubmit()");
