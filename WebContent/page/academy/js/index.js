...
testonline.academy= {
	...
	init : function(){
		...
		/*testonline.academy.setSearchBtn();*/
		/*$.ajax({
			type:'post',
...
//					"'waterHeight':56,'takeUpAcreage':80000,'typhoonLevel':12,'tideType':'半日潮','seareaType':'内海','feedClass':'大鱼'}",page:1,rows:10}
	...
	setDataGrig : function(){
		$('#data').datagrid({  
            ...
            columns:[[
                      {field:'academyId',title:'编号',width:100,align:'center'},
                      {field:'academyName',title:'名称',width:100,align:'center'},
                      {field:'academyDesc',title:'描述',width:100,align:'center'},
                  ]],
            pagination:true,
            pageSize:15,
            ...
            onLoadSuccess : function(data){
            	...
            	if(data.total == 0){
            		if(testonline.academy.searchStat == '查询'){
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">您查询的学院信息不存在！</p>');
            			testonline.academy.searchStat='';
            		}else{
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">暂无学院信息！</p>');
            		}
            	}else{
				...
				$.ajax({
					...
					success : function(result){
						if(result.success){
							...
							$('#fm').form('clear');
							$("#submitBtn").attr("onclick","testonline.academy.editSubmit()");
