...
testonline.classes= {
	...
	init : function(){
		...
		/*testonline.classes.setSearchBtn();*/
		/*$.ajax({
			type:'post',
...
//					"'waterHeight':56,'takeUpAcreage':80000,'typhoonLevel':12,'tideType':'半日潮','seareaType':'内海','feedClass':'大鱼'}",page:1,rows:10}
	...
	setDataGrig : function(){
		$('#data').datagrid({  
            ...
            columns:[[
                      {field:'classId',title:'编号',width:100,align:'center'},
                      {field:'className',title:'班别',width:100,align:'center'},
                      {field:'grade',title:'年级',width:100,align:'center'},
                      {field:'majorName',title:'专业',width:100,align:'center',formatter:function(value,rec){  
            ...
            onLoadSuccess : function(data){
            	...
            	if(data.total == 0){
            		if(testonline.classes.searchStat == '查询'){
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">您查询的班级信息不存在！</p>');
            			testonline.classes.searchStat='';
            		}else{
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">暂无班级信息！</p>');
            		}
            	}else{
				...
				$.ajax({
					...
					success : function(result){
						if(result.success){
							...
							$('#fm').form('clear');
							$("#submitBtn").attr("onclick","testonline.classes.editSubmit()");
