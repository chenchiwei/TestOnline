...
testonline.question= {
	...
	init : function(){
		...
		/*testonline.question.setSearchBtn();*/
		/*$.ajax({
			type:'post',
...
//					"'waterHeight':56,'takeUpAcreage':80000,'typhoonLevel':12,'tideType':'半日潮','seareaType':'内海','feedClass':'大鱼'}",page:1,rows:10}
	...
	setDataGrig : function(){
		$('#data').datagrid({  
            ...
            columns:[[
                      {field:'questionId',title:'编号',width:100,align:'center'},
                      {field:'questionContent',title:'试题',width:100,align:'center'},
                      {field:'questionType',title:'类型',width:100,align:'center'},
                      {field:'questionLevel',title:'难度',width:100,align:'center'},
            ...
            onLoadSuccess : function(data){
            	...
            	if(data.total == 0){
            		if(testonline.question.searchStat == '查询'){
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">您查询的试题信息不存在！</p>');
            			testonline.question.searchStat='';
            		}else{
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">暂无试题信息！</p>');
            		}
            	}else{
	...
	setImportBtn : function(){
		$('#importbtn').click(function(){
			$('#importfm').form('clear');
			$("#submitBtn").attr("onclick","testonline.question.importSubmit()");
	...
	setEditBtn : function(){
		$('#editbtn').click(function(){
			...
			if(rows.length!=1){
				...
				$.ajax({
					url : getRootpath()+"/question/getById/"+id,
					type : 'POST',
					dataType:'json',
					success : function(result){
						if(result.success){
							...
							$('#fm').form('clear');
							$("#submitBtn").attr("onclick","testonline.question.editSubmit()");
							$("#questionId").val(question.questionId);
							$("#questionContent").val(question.questionContent);
							
							$('#adddlg').dialog('open','编辑试题信息');
						}else{
							$.messager.alert(
								 '提示',
								 "加载失败！"
		...
		if($("#importfm").form('validate')){
			$.messager.confirm('提交窗口？',"你确定要提交吗？",function(result){
				if(result){
					...
					if(courseId){
						var options = {
								url : getRootpath() + "/question/importExcel/"+courseId,
								type : "POST",
								success : function(result) {
