...
testonline.teacher= {
	...
	init : function(){
		...
		$("#toolbar1").hide();
	//	testonline.teacher.academyChange();
		/*testonline.teacher.setSearchBtn();*/
		/*$.ajax({
			type:'post',
...
//					"'waterHeight':56,'takeUpAcreage':80000,'typhoonLevel':12,'tideType':'半日潮','seareaType':'内海','feedClass':'大鱼'}",page:1,rows:10}
	...
	setDataGrig : function(){
		$('#data').datagrid({  
            ...
            columns:[[
                      {field:'teaId',title:'编号',width:100,align:'center'},
                      ...
                      {field:'teaEmail',title:'邮箱',width:100,align:'center'},
                      {field:'teaPhone',title:'手机',width:100,align:'center'}
                  ]],
            pagination:true,
            pageSize:15,
            ...
            onLoadSuccess : function(data){
            	...
            	if(data.total == 0){
            		if(testonline.teacher.searchStat == '查询'){
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">您查询的教师信息不存在！</p>');
            			testonline.teacher.searchStat='';
            		}else{
            			$('.datagrid-body').eq(1).html('').append('<p style="text-align:center; line-height:280px;">暂无教师信息！</p>');
            		}
            	}else{
	...
	academyChange:function(){
		$("#academy").change(function(){
			var academyId=this.value;
			var select=$("#class");
			$("#class").children().remove();
			$.ajax({
				url : getRootpath()+"/class/findByAcademyId/"+academyId,
				type : 'POST',
				dataType:'json',
				...
				$.ajax({
					...
					success : function(result){
						if(result.success){
							...
							$('#fm').form('clear');
							$("#submitBtn").attr("onclick","testonline.teacher.editSubmit()");
					...
					.datagrid(
							{
								...
								striped : true,
								url : getRootpath()
										+ "/class/findByAcademyId/"+academyId,
								queryParams : {
									jsondata : "{'title':''}"
								},
								toolbar : '#toolbar1',
	...
	submitSelect:function(){
		...
		if(ids.length<=0){
			...
			$.messager.confirm('提交窗口？',"你确定要分配选中的几个班级吗？",function(result){
				if(result){
					$.ajax({
						url : getRootpath()+"/teacher/forClass/"+$testonlineteaId,
						type : 'POST',
						dataType:'json',
						contentType : "application/json",//这个必须要加
						data : JSON.stringify(ids),
